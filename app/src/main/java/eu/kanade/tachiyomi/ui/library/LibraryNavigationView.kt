...
class LibraryNavigationView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null)
     ...
     */
    private val groups = listOf(FilterGroup(), SortGroup(), DisplayGroup(), BadgeGroup(), AllGroup())
    ...
    inner class AllGroup : Group {
        private val allCategory = Item.CheckboxGroup(R.string.action_display_all_category, this)
        override val header = null
        override val footer = null
        override val items = listOf(allCategory)
        override fun initModels() {
            allCategory.checked = preferences.showAllCategory().getOrDefault()
        }

        override fun onItemClicked(item: Item) {
            item as Item.CheckboxGroup
            item.checked = !item.checked
            preferences.showAllCategory().set((item.checked))
            adapter.notifyItemChanged(item)
        }
    }
